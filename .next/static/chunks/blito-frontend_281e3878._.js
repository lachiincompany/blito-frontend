        ...
        "BookingPage.useEffect": ()=>{
            const fetchData = {
                "BookingPage.useEffect.fetchData": async ()=>{
                    ...
                    try {
                        // 1. دریافت جزئیات سفر
                        const tripRes = await fetch("http://localhost:8000/trips/api/v1/trips/".concat(tripId, "/"));
                        if (!tripRes.ok) throw new Error('خطا در دریافت اطلاعات سفر');
                        const tripData = await tripRes.json();
                        setTripDetails(tripData);
                        // 2. دریافت لیست صندلی‌ها
                        const token = localStorage.getItem('accessToken');
                        const seatRes = await fetch("http://localhost:8000/seat/api/v1/api/seats/trip/".concat(tripId, "/"), {
                            headers: token ? {
